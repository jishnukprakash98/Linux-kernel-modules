...
static int __init start_program(void)
{
	...
	if(strcmp(proc_any,"HELL")==0)
	{
	
		printk(KERN_INFO "\n\nThere is no input paramter, Nothing to pause\n\n");
	...
	if(cp_task != NULL) 
	{
		...
		if(check == 1) //If the input parameter is PID then this section will be executed.
		{
			...
			for_each_process(task) 
			{
				...
				//printk("Checking process : %s with PID : %d for match", proc_pid, task->pid);
				/* Check if task->comm starts with 'cp ' */
				if(task->pid == proc_pid) 
				{
				
					printk(KERN_INFO "\n\n%s found with pid : %d and RESUMED \n\n" ,task->comm, task->pid);
					/* Resume the cp process when unloading module. */
							
					//printk(KERN_INFO "Resuming above process.");
		...
		else
		{
			for_each_process(task) 
			{
				...
				//printk("Checking process : %s with PID : %d for match", task->comm, proc_any);
				/* Check if task->comm starts with 'cp ' */
				if(strcmp(task->comm, proc_any) == 0) 
				{
