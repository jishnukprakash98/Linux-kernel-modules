	...
	do {
		...
		switch(choice) {
	   	...
	   	case 1 : Clearmem();
	   		 AllProcess();
   			 temp2 = head2; 
   			 printf("\n" YELLOW "  %-17s" GREEN "%18s" CYAN "%8s" RED "%13s" RESET "\n\n",
			"Process Name","Process Id","State","Priority");		
	
   			 while(temp2 != NULL) {
	   	 	        printf(YELLOW "  %-17s" GREEN "%14d" CYAN "%10c" RED "%12d" RESET "\n",
...
int AllProcess()
{
	...
	if ((dir = opendir("/proc/")) == NULL) {
		perror("opendir(/proc/)");	// Print reason for failure of opendir to stderr.
		return 0;
	}
  
	while ((dentry = readdir(dir)) != NULL) 
    	{
		strcpy(name2,dentry->d_name);
		pid = atoi(name2);
    	   	if(pid != 0)
		{
...
/* This is the function to pause the ongoing processes withn the system,
 * the paused processes are linearly added to a growing singly linked list.
*/
int Paused()
 {
	char	a, pause[20];
  	char	process_name[70], var[100];
	int	flag;
	int	flag2;
	int	pid;
	...
	repeat:	flag2 = 0;
		...
		if(isdigit(var[0]))
		{
			...
			if(flag == 0)
			{
				...
				{
					perror("kill(pid,"
						"SIGSTOP)"); // Print reason for failure of kill(pid)
					// goto repeat; // Results in asking indefinitely until a valid pid is given :(.
					goto ask;	// Asks if user wants to continue. 
				}
			...
			temp = head;
		 	...
		 	while(temp2 != NULL)
		 	{
		 		if((strcmp(temp2->procname,var) == 0))
		 		{
		 			flag2 = 1;
		 			temp = (struct node*)malloc(sizeof(struct node));
		 			if (kill(temp2->processid,SIGSTOP) == -1)
						perror("kill(temp2->processid"
							"SIGSTOP)");
		 	...
		 	if(flag2 == 0)
		 	{
		 		printf(RED"\nEntered name corresponds to a " YELLOW "Nonexistant"
					RED" process"RESET"\n");
		 		goto repeat;
			}
		}
...
/* This is a function to check exceptional conditions
 * as in the scenarios when a user tries to pause an 
 * already paused process or resume a non paused process etc..... 
 */
int Check(int pid)
{
	if(head == NULL)
			 	...
			 	while(temp2 != NULL)
			 	{
			 		if((strcmp(temp2->procname,var) == 0))
			 		{
					 	...
					 	while(current != NULL)
					 	{
					 		...
					 		{
					 			if (kill(current->data,SIGCONT) == -1)
									perror("kill(current->data,"
										"SIGCONT)");
					 			head = head->link;
					 			temp = current = head;
					 		}
