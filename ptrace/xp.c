...
main(int argc, char **argv)
{
   ...
   // waitpid(pid, 0, 0); // sync with PTRACE_TRACEME
	printf("Attaching to %ld",ptrace(PTRACE_SEIZE, 1767, 0, 0));
    //ptrace(PTRACE_SETOPTIONS, 1767, 0, PTRACE_O_TRACEFORK);  //Stop the tracee at the next execve
	ptrace(PTRACE_INTERRUPT,1767,0);
	waitpid(1767, 0,WNOHANG);
    for (;;) {
        // Enter next system call 
        if (ptrace(PTRACE_SYSCALL, 1767, 0, 0) == -1) {
            FATAL("%s", strerror(errno));
        }
        ...
        if (ptrace(PTRACE_GETREGS, 1767, 0, &regs) == -1)
            FATAL("%s", strerror(errno));

        if (is_syscall_blocked(regs.orig_rax)) {
            regs.orig_rax = -1; // set to invalid system call
            if (ptrace(PTRACE_SETREGS, 1767, 0, &regs) == -1)
                FATAL("%s", strerror(errno));
        }
        ...
        switch (regs.orig_rax) {
            ...
            case SYS_xpledge:
                ...
                break;
			case SYS_fork:	printf("\nIntercepted fork() call\n");
							break;
			case SYS_vfork: printf("\nIntercepted vfork() call\n");
							break;
        }
